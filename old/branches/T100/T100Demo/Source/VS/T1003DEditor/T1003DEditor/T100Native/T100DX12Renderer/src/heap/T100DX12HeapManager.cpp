...
T100VOID T100DX12HeapManager::Start(T1003DConfig* config)
{
    m_rtvHeapDispenser.Create(m_space->GetDevicePtr(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV, m_space->GetFrameCount());
    m_cbvsrvHeapDispenser.Create(m_space->GetDevicePtr(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, config->CbvSrvHeapSize);
    m_dsvHeapDispenser.Create(m_space->GetDevicePtr(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV, config->DsvHeapSize);
    m_samplerHeapDispenser.Create(m_space->GetDevicePtr(), D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, config->SamplerHeapSize);
}
