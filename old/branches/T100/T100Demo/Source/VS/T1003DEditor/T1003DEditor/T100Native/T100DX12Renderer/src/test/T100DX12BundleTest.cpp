...
T100DX12BundleTest::T100DX12BundleTest()
{
    //ctor
}
...
T100VOID T100DX12BundleTest::init(HWND hwnd)
{
    ...
    T1003DCamera* camera = T100NEW T1003DCamera(T1003D_COORDINATE_SYSTEM_RIGHT_HAND);

    camera->SetFov(0.8f);
    camera->SetAspectRatio(m_width * 1.0f / m_height);
    camera->SetPosition(10, 5, 30);
...
T100VOID T100DX12BundleTest::update()
{
    m_timer.Tick();
    ...
    m_camera.GetProjectionMatrix(project);
    m_pCurrentFrameResource->UpdateConstantBuffers(view, project);

}

T100VOID T100DX12BundleTest::render()
{
...
T100VOID T100DX12BundleTest::LoadPipeline()
{
...
T100VOID T100DX12BundleTest::LoadAssets()
{
    ...
    UINT8* pMeshData;
    UINT meshDataLength;
    ThrowIfFailed(ReadDataFromFile(GetAssetFullPath(DataFileName).c_str(), &pMeshData, &meshDataLength));
    //ThrowIfFailed(ReadDataFromFile(DataFileName, &pMeshData, &meshDataLength));
...
void T100DX12BundleTest::CreateFrameResources()
{
    ...
    for (UINT i = 0; i < FrameCount; i++)
    {
        T100DX12FrameResourceTest* pFrameResource = new T100DX12FrameResourceTest(m_device.Get(), CityRowCount, CityColumnCount);
        ...
        for (UINT j = 0; j < CityRowCount; j++)
        {
            for (UINT k = 0; k < CityColumnCount; k++)
            {
                ...
                cbvDesc.SizeInBytes = sizeof(T100DX12FrameResourceTest::SceneConstantBuffer);
...
void T100DX12BundleTest::PopulateCommandList(T100DX12FrameResourceTest* pFrameResource)
{
...
std::wstring T100DX12BundleTest::GetAssetFullPath(LPCWSTR assetName)
{
    return m_assetsPath + L"..\\..\\resources\\entity\\" + assetName;
}


_Use_decl_annotations_
void T100DX12BundleTest::GetHardwareAdapter(
