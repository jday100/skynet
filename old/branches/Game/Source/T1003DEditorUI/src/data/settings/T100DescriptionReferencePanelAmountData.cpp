...
T100DescriptionReferencePanelAmountData::T100DescriptionReferencePanelAmountData(wxPropertyGrid* grid, T1003DEditorDescriptionReferenceData* desc, T100UINT index) :
...
T100DescriptionReferencePanelAmountData::~T100DescriptionReferencePanelAmountData()
{
    //dtor
}

T100VOID T100DescriptionReferencePanelAmountData::Update(wxPGProperty* property)
{
    ...
    if(value){
        m_desc->m_descs.resize(value);
    }

    CreateDescriptions();
}

T100VOID T100DescriptionReferencePanelAmountData::CreateDescriptions()
{
    T100UINT    value   = m_desc->Amount;
    ...
    for(T100UINT i = 0; i < value; i++){
        CreateDescription(enumLabels, enumValues, m_desc->m_descs, i);
    }
}


T100VOID T100DescriptionReferencePanelAmountData::CreateDescription(wxArrayString& labels, wxArrayInt& values, wxPGProperty* parent, T100UINT index)
{
...
T100VOID T100DescriptionReferencePanelAmountData::RemoveDescriptions()
{
    if(m_desc->Amount){
        m_desc->m_descs.clear();
    }

    for(T1003DEDITOR_GRID_DATA_DESCRIPTION& description : m_descriptionProperties){
