#include "T100DX12Discrete.h"

#include "T100DX12Tools.h"
#include "T100DX12Studio.h"
#include "T100City.h"

T100DX12Discrete::T100DX12Discrete(T100DX12Studio* studio) :
    m_studio(studio)
{
    //ctor
}

T100DX12Discrete::~T100DX12Discrete()
{
    //dtor
}

T100VOID T100DX12Discrete::Start()
{
    LoadAssets();
    CreateFrameResources();
}

T100VOID T100DX12Discrete::Update()
{
    BundleUpdate();
}

T100VOID T100DX12Discrete::Render()
{
    BundleRender();
}

T100VOID T100DX12Discrete::LoadAssets()
{
    m_entity    = T100NEW T100City();
    m_model     = T100NEW T100DX12Model();
    m_context   = T100NEW T100DX12Context();

    m_context->m_vertexBufferViewPtr    = &m_model->m_vertexBufferView;
    m_context->m_indexBufferViewPtr     = &m_model->m_indexBufferView;
...

    {
        ...
        CreatePipelineState1And2(pVertexShaderData, vertexShaderDataLength,
    ...
    CreateVertexBuffer(pMeshData, vertexBufferUploadHeap);
    CreateIndexBuffer(pMeshData, indexBufferUploadHeap);
    CreateTextureBuffer(pMeshData, textureUploadHeap);
...
T100VOID T100DX12Discrete::CreateFrameResources()
{
    ...
    for (UINT i = 0; i < m_studio->m_frameCount; i++)
    {
        T100DX12FrameResource* pFrameResource = m_frameResources[i];
        pFrameResource->Create(m_studio->m_device.Get(), m_entity->CityRowCount, m_entity->CityColumnCount);

        CreateConstantBufferView(cbvSrvHandle, pFrameResource);

        pFrameResource->InitBundle(m_studio->m_device.Get(), m_context->m_pipelineState1.Get(), m_context->m_pipelineState2.Get(), i, m_context->m_numIndices, m_context->m_indexBufferViewPtr,
            m_context->m_vertexBufferViewPtr, m_studio->m_heap_manager.GetCbvSrvHeap().m_heap.m_heap.Get(), m_studio->m_heap_manager.GetCbvSrvHeap().m_heap.m_heapDescriptorSize,
            m_studio->m_heap_manager.GetSamplerHeap().m_heap.m_heap.Get(), m_rootSignature.Get());

    }
}

T100VOID T100DX12Discrete::PopulateCommandList(T100DX12FrameResource* frameResource)
{

    ThrowIfFailed(pCurrentFrameResource->m_commandAllocator->Reset());

    PopulateContext(frameResource);
}


T100VOID T100DX12Discrete::PopulateContext(T100DX12FrameResource* frameResource)
{
    ThrowIfFailed(m_context->m_commandList->Reset(frameResource->m_commandAllocator.Get(), m_context->m_pipelineState1.Get()));

    m_context->m_commandList->SetGraphicsRootSignature(m_rootSignature.Get());

    ID3D12DescriptorHeap* ppHeaps[] = {
        m_studio->m_heap_manager.GetCbvSrvHeap().m_heap.m_heap.Get(),
        m_studio->m_heap_manager.GetSamplerHeap().m_heap.m_heap.Get() };
    m_context->m_commandList->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);

    m_context->m_commandList->RSSetViewports(1, &m_studio->m_viewport);
    m_context->m_commandList->RSSetScissorRects(1, &m_studio->m_scissorRect);
    ...
    m_context->m_commandList->OMSetRenderTargets(1, &rtvHandle, FALSE, &dsvHandle);

    const float         clearColor[] = { 0.0f, 0.2f, 0.4f, 1.0f };
    m_context->m_commandList->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr);
    m_context->m_commandList->ClearDepthStencilView(m_studio->m_heap_manager.GetDsvHeap().GetCPUStart(), D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr);

    if (m_entity->m_bundle.USED)
    {
        m_context->m_commandList->ExecuteBundle(frameResource->m_bundle.Get());
    }
    else
    {
        frameResource->PopulateCommandList(m_context->m_commandList.Get(), m_context->m_pipelineState1.Get(), m_context->m_pipelineState2.Get(), m_currentFrameResourceIndex, m_context->m_numIndices, m_context->m_indexBufferViewPtr,
            m_context->m_vertexBufferViewPtr, m_studio->m_heap_manager.GetCbvSrvHeap().m_heap.m_heap.Get(), m_studio->m_heap_manager.GetCbvSrvHeap().m_heap.m_heapDescriptorSize,
            m_studio->m_heap_manager.GetSamplerHeap().m_heap.m_heap.Get(), m_rootSignature.Get());
    }
    ...
    m_context->m_commandList->ResourceBarrier(1, &present_barrier);

    ThrowIfFailed(m_context->m_commandList->Close());
}

T100VOID T100DX12Discrete::BundleUpdate()
{
...
T100VOID T100DX12Discrete::BundleRender()
{
    PopulateCommandList(pCurrentFrameResource);

    ID3D12CommandList*          ppCommandLists[] = { m_context->m_commandList.Get() };
...
T100VOID T100DX12Discrete::CreatePipelineState1And2(
    UINT8* pVertexShaderData, UINT vertexShaderDataLength,
    UINT8* pPixelShaderData1, UINT pixelShaderDataLength1,
    UINT8* pPixelShaderData2, UINT pixelShaderDataLength2)
{
        ...
        ThrowIfFailed(m_studio->m_device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_context->m_pipelineState1)));

        psoDesc.PS = CD3DX12_SHADER_BYTECODE(pPixelShaderData2, pixelShaderDataLength2);

        ThrowIfFailed(m_studio->m_device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_context->m_pipelineState2)));

}

T100VOID T100DX12Discrete::CreateCommandList()
{
    ThrowIfFailed(m_studio->m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator.Get(), nullptr, IID_PPV_ARGS(&m_context->m_commandList)));
}

T100VOID T100DX12Discrete::CreateRenderTargetView()
{
...
T100VOID T100DX12Discrete::CreateDepthStencilView()
{
...
T100VOID T100DX12Discrete::CloseCommandList()
{
    ThrowIfFailed(m_context->m_commandList->Close());
}

T100VOID T100DX12Discrete::ExecuteCommandList()
{
    ID3D12CommandList*                  ppCommandLists[] = { m_context->m_commandList.Get() };
    m_studio->m_commandQueue->ExecuteCommandLists(_countof(ppCommandLists), ppCommandLists);
...
T100VOID T100DX12Discrete::CreateFence()
{
...
T100VOID T100DX12Discrete::Waiting()
{
...
T100VOID T100DX12Discrete::CreateVertexBuffer(UINT8* pMeshData, ComPtr<ID3D12Resource>& vertexBufferUploadHeap)
{
        ...
        ThrowIfFailed(m_studio->m_device->CreateCommittedResource(
            ...
            nullptr,
            IID_PPV_ARGS(&m_model->m_vertexBuffer)));
        ...
        UpdateSubresources<1>(m_context->m_commandList.Get(), m_model->m_vertexBuffer.Get(), vertexBufferUploadHeap.Get(), 0, 0, 1, &vertexData);
        CD3DX12_RESOURCE_BARRIER        barrier     = CD3DX12_RESOURCE_BARRIER::Transition(m_model->m_vertexBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
        m_context->m_commandList->ResourceBarrier(1, &barrier);

        m_context->m_vertexBufferViewPtr->BufferLocation   = m_model->m_vertexBuffer->GetGPUVirtualAddress();
        m_context->m_vertexBufferViewPtr->StrideInBytes    = StandardVertexStride;
        m_context->m_vertexBufferViewPtr->SizeInBytes      = VertexDataSize;

}


T100VOID T100DX12Discrete::CreateIndexBuffer(UINT8* pMeshData, ComPtr<ID3D12Resource>& indexBufferUploadHeap)
{
        ...
        ThrowIfFailed(m_studio->m_device->CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_COPY_DEST,
            nullptr,
            IID_PPV_ARGS(&m_model->m_indexBuffer)));
        ...
        UpdateSubresources<1>(m_context->m_commandList.Get(), m_model->m_indexBuffer.Get(), indexBufferUploadHeap.Get(), 0, 0, 1, &indexData);
        CD3DX12_RESOURCE_BARRIER        barrier         = CD3DX12_RESOURCE_BARRIER::Transition(m_model->m_indexBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_INDEX_BUFFER);
        m_context->m_commandList->ResourceBarrier(1, &barrier);

        m_context->m_indexBufferViewPtr->BufferLocation    = m_model->m_indexBuffer->GetGPUVirtualAddress();
        m_context->m_indexBufferViewPtr->Format            = m_studio->GetModelManager().ConvertFormat(StandardIndexFormat);
        m_context->m_indexBufferViewPtr->SizeInBytes       = IndexDataSize;

        m_context->m_numIndices = IndexDataSize / 4;

}

T100VOID T100DX12Discrete::CreateTextureBuffer(UINT8* pMeshData, ComPtr<ID3D12Resource>& textureUploadHeap)
{
        ...
        ThrowIfFailed(m_studio->m_device->CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_COPY_DEST,
            nullptr,
            IID_PPV_ARGS(&m_model->m_texture)));

        const UINT subresourceCount     = textureDesc.DepthOrArraySize * textureDesc.MipLevels;
        const UINT64 uploadBufferSize   = GetRequiredIntermediateSize(m_model->m_texture.Get(), 0, subresourceCount);
        ...
        UpdateSubresources(m_context->m_commandList.Get(), m_model->m_texture.Get(), textureUploadHeap.Get(), 0, 0, subresourceCount, &textureData);
        CD3DX12_RESOURCE_BARRIER        barrier         = CD3DX12_RESOURCE_BARRIER::Transition(m_model->m_texture.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        m_context->m_commandList->ResourceBarrier(1, &barrier);
        ...
        m_studio->m_device->CreateShaderResourceView(m_model->m_texture.Get(), &srvDesc, m_studio->m_heap_manager.GetCbvSrvHeap().GetCPUStart());

}

T100VOID T100DX12Discrete::CreateConstantBufferView(CD3DX12_CPU_DESCRIPTOR_HANDLE& cbvSrvHandle, T100DX12FrameResource* pFrameResource)
{
    UINT64 cbOffset = 0;
...
DXGI_FORMAT T100DX12Discrete::ConvertFormat(T100DXGI_FORMAT format)
{
