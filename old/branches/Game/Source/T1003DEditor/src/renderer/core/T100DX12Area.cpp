...
T100VOID T100DX12Area::Create(HWND hwnd, UINT width, UINT height)
{
    m_hwnd      = hwnd;
...
T100VOID T100DX12Area::SetSize(UINT width, UINT height)
{
    m_width     = width;
    m_height    = height;
...
{
    CD3DX12_CPU_DESCRIPTOR_HANDLE       rtvHandle = m_heap_manager.GetRtvHeap().GetCPUStart();

    for(UINT n = 0; n < m_frameCount; n++)
    {
        ThrowIfFailed(m_swapChain->GetBuffer(n, IID_PPV_ARGS(&m_renderTargets[n])));
        m_device->CreateRenderTargetView(m_renderTargets[n].Get(), T100NULL, rtvHandle);
        rtvHandle.Offset(1, m_heap_manager.GetRtvHeap().m_heap.m_heapDescriptorSize);
    }
}
...
T100VOID T100DX12Area::CreateDepthStencilView()
{
    ...
    m_device->CreateDepthStencilView(m_depthStencil.Get(), &depthStencilDesc, m_heap_manager.GetDsvHeap().m_heap.m_heap->GetCPUDescriptorHandleForHeapStart());
}
