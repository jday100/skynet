#include "T100DX12ContextRectangle.h"
...
T100DX12ContextRectangle::T100DX12ContextRectangle() :
    T100DX12Context()
{
    //ctor
}
...
T100VOID T100DX12ContextRectangle::Create()
{
    if(Bundle.USED){
        CreateBundle();
    }else{
...
T100VOID T100DX12ContextRectangle::Destroy()
{

}

T100VOID T100DX12ContextRectangle::Update()
{
    if(Bundle.USED){
        UpdateBundle();
    }else{
...
T100VOID T100DX12ContextRectangle::Render()
{
    if(Bundle.USED){
        RenderBundle();
    }else{
...
T100VOID T100DX12ContextRectangle::CreateSingle()
{
...
T100VOID T100DX12ContextRectangle::UpdateSingle()
{
    XMMATRIX        projection;
    XMMATRIX        view;
    XMMATRIX        model;
    ...
    //PRINTMATRIX("UpdateSingle matrix:", result);
...
T100VOID T100DX12ContextRectangle::RenderSingle()
{
...
T100VOID T100DX12ContextRectangle::CreateBundle()
{
    ...
    ThrowIfFailed(m_studioPtr->GetDevicePtr()->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_BUNDLE, m_bundleAllocator.Get(), m_pipelineState1.Get(), IID_PPV_ARGS(&m_bundleCommandList)));
    ...
    CD3DX12_GPU_DESCRIPTOR_HANDLE textureHandle(
        m_studioPtr->GetHeapManager().GetCbvSrvHeap().GetGPUStart(), m_textureHeapIndex,
        m_studioPtr->GetHeapManager().GetCbvSrvHeap().GetSize());
    CD3DX12_GPU_DESCRIPTOR_HANDLE samplerHandle(
        m_studioPtr->GetHeapManager().GetSamplerHeap().GetGPUStart(), m_samplerHeapIndex,
        m_studioPtr->GetHeapManager().GetSamplerHeap().GetSize());

    m_bundleCommandList->SetGraphicsRootDescriptorTable(0, textureHandle);
    m_bundleCommandList->SetGraphicsRootDescriptorTable(1, samplerHandle);
    ...
    for (UINT i = 0; i < Bundle.AMOUNT.X; i++)
    {
        for (UINT j = 0; j < Bundle.AMOUNT.Y; j++)
        {
            ...
            {
                m_bundleCommandList->SetGraphicsRoot32BitConstant(3, (k * Bundle.AMOUNT.X) + i, 0);

                m_bundleCommandList->SetPipelineState(usePso1 ? m_pipelineState.Get() : m_pipelineState.Get());
                usePso1 = !usePso1;

                m_bundleCommandList->SetGraphicsRootDescriptorTable(2, cbvSrvHandle2);
                cbvSrvHandle2.Offset(m_studioPtr->GetHeapManager().GetCbvSrvHeap().GetSize());

                m_bundleCommandList->DrawIndexedInstanced(m_numIndices, 1, 0, 0, 0);
            }
        }
    }
...
T100VOID T100DX12ContextRectangle::UpdateBundle()
{
    ...
    for (UINT i = 0; i < Bundle.AMOUNT.X; i++)
    {
        for (UINT j = 0; j < Bundle.AMOUNT.Y; j++)
        {
            for (UINT k = 0; k < Bundle.AMOUNT.Z; k++)
            {
                ...
                //PRINTMATRIX("UpdateBundle matrix:", result);
...
T100VOID T100DX12ContextRectangle::RenderBundle()
{
    ...
    CD3DX12_CPU_DESCRIPTOR_HANDLE dsvHandle(
        m_studioPtr->GetHeapManager().GetDsvHeap().GetCPUStart(), m_framePtr->m_index,
        m_studioPtr->GetHeapManager().GetDsvHeap().GetSize());
