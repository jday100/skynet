#include "T100DX12HeapDispenser.h"
...
T100VOID T100DX12HeapDispenser::Create(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE type, UINT length)
{
    switch(type){
...
{
    return m_heap.m_heap.Get();
}

UINT T100DX12HeapDispenser::GetSize()
{
    return m_heap.m_heapDescriptorSize;
}

UINT T100DX12HeapDispenser::GetTop()
{
    return m_top;
}
...
{
    T100UINT value = m_heap.m_length;
    ...
    m_current   = CD3DX12_CPU_DESCRIPTOR_HANDLE(m_current, length, m_heap.m_heapDescriptorSize);
    m_top       += length;
    return result;
}

CD3DX12_CPU_DESCRIPTOR_HANDLE T100DX12HeapDispenser::GetCurrent()
{
    return m_current;
}
...
T100VOID T100DX12HeapDispenser::CreateRtvHeap(ID3D12Device* device, T100DX12Heap& heap, UINT length)
{
    ...
    heap.m_heapDescriptorSize = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
}
...
T100VOID T100DX12HeapDispenser::CreateCbvSrvHeap(ID3D12Device* device, T100DX12Heap& heap, UINT length)
{
    ...
    heap.m_heapDescriptorSize = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
}
