...
std::ifstream& operator>>(T1003DFileReader& reader, T100JSON& json)
{
    *reader.m_ifstream >> json;
    return *reader.m_ifstream;
}

std::ifstream& operator>>(T1003DFileReader& reader, T100File3DF& file)
{
    *reader.m_ifstream >> file.GetJSON().GetJSON();
    return *reader.m_ifstream;
}
