...
#error This file is not compatible with the current WINAPI_FAILY
#endif
...
_Success_(return != false)
inline bool CTime::GetAsDBTIMESTAMP(_Out_ DBTIMESTAMP& dbts) const throw()
{
    struct tm tmLocal;
...
_Success_(return != NULL)
inline struct tm* CTime::GetGmtTm(_Out_ struct tm* ptm) const
{
    ATLENSURE( ptm != NULL );
...
_Success_(return != NULL)
inline struct tm* CTime::GetLocalTm(_Out_ struct tm* ptm) const
{
    ATLENSURE( ptm != NULL );
...
_Success_(return != false)
inline bool CTime::GetAsSystemTime(_Out_ SYSTEMTIME& timeDest) const throw()
{
    struct tm ttm;
    struct tm* ptm;
...
inline CString CTimeSpan::Format(_In_z_ LPCTSTR pFormat) const
{
    ...
    {
        enum _CTIMESPANFORMATSETP formatstep = _CTFS_NONE;
        if(ch == _T('%'))
        {
        ...
        case 'M':
            ...
            break;
        case 'S':
            strBuffer.AppendFormt(hmsFormats[formatstep], formatstep ? GetSeconds() : ch);
            break;
        default:
            if(formatstep)
            {
    ...
    _In_ int nDST) throw()
{
    ...
    atm.tm_mon      = dbts.month - 1;
    ATLASSERT(dbts.year >= 1900);
    atm.tm_year     = dbts.year - 1900;
    atm.tm_isdst    = nDST;
    m_time          = _mktime64(&atm);
    ATLASSUME(m_time != -1);
}
