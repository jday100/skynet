...
T100Sentence::T100Sentence(T100SentenceScan* scan)
    :m_scan(scan)
{
    //ctor
    m_token     = m_scan->m_token;
    m_item      = &(m_scan->m_item);
    m_loaded    = &(m_scan->m_loaded);
...
T100STDSTRING T100Sentence::tostring()
{
    return data.to_std_string();
}
    ...
    case T100CONSTANT_INTEGER:
        {
            ...
            op.VALUE        = std::stoll(m_item->data.to_std_string().c_str(), T100NULL, 0);
    ...
    case T100CONSTANT_FLOAT:
        {
            ...
            op.VALUE        = std::stof(m_item->data.to_std_string().c_str());
            setLoaded(T100FALSE);
    ...
    case T100CHAR_OPEN_BRACKETS:
        {
            ...
            if(T100CONSTANT_INTEGER == m_item->type){
                ...
                value = std::stoll(m_item->data.to_std_string());
...
T100BOOL T100Sentence::run(T100FileData* code)
{
    T100BOOL    result;
...
{
    //T100Log::info(T100AssemblyHint::build_hint(type, data, T100BUILD_SENTENCE_SUCCESS));
    T100Log::info(T100AssemblyHint::build_hint(type, tostring(), T100BUILD_SENTENCE_SUCCESS));
}

T100BOOL T100Sentence::build(T100FileData* code)
{
    T100AssemblyError::error(T100AssemblyHint::build_hint(type, data, T100BUILD_SENTENCE_ERROR));
    return T100FALSE;
}

T100BOOL T100Sentence::buildOperator(T100FileData* code, T100OPERATOR& target, T100OPERATOR_BUILD& info)
{
    T100BOOL    result          = T100FALSE;
    ...
    case T100DATA_VARIABLE:
        {
            ...
            T100VARIABLE_DEFINE* vd = T100Code::getVariableDefine(target.NAME);
            if(!vd){
                T100AssemblyError::error(T100AssemblyHint::build_hint(type, data, T100BUILD_SENTENCE_ERROR));
                return T100FALSE;
            }
...
T100BOOL T100Sentence::buildOperator(T100FileData* code, T100OPERATOR_COMPLEXUS& op, T100OPERATOR_BUILD& info)
{
    T100BOOL    result          = T100FALSE;
        ...
        case T100DATA_VARIABLE:
            {
                ...
                T100VARIABLE_DEFINE* vd = T100Code::getVariableDefine(op.BASE.NAME);
                if(!vd){
                    T100AssemblyError::error(T100AssemblyHint::build_hint(type, data, T100BUILD_SENTENCE_ERROR));
                    return T100FALSE;
                }
...
T100BOOL T100Sentence::buildOperator(T100FileData* code, T100OPERATOR_BINOCULAR& op, T100OPERATOR_INFO& info)
{
    T100BOOL    result          = T100FALSE;
...
T100BOOL T100Sentence::buildComplexus(T100FileData* code, T100OPERATOR_COMPLEXUS& op, T100OPERATOR_BUILD& info)
{
    T100BOOL    result          = T100FALSE;
...
T100BOOL T100Sentence::buildInfo(T100ORDER_TYPE type, T100FileData* code, T100OPERATOR_INFO& info)
{
