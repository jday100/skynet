...
T100BOOL T100VFSItem::list(T100DWORD current, T100DISK_ITEM_VECTOR& items)
{
    ...
    for(int i=0;i<16 * 16 - 1;i++){
        if(1 == info.ITEMS[i].PROPERTIES[0]){
            T100Library::T100DISK_ITEM*  item = T100NEW T100Library::T100DISK_ITEM();

            item->NAME      = T100Library::T100String32Tools::to_string32(info.ITEMS[i].NAME, 202);
...
T100BOOL T100VFSItem::search(T100STRING name, T100VFS_ITEM_HANDLE& handle)
{
    ...
    for(int i=0;i<16 * 16 - 1;i++){
        if(1 == info.ITEMS[i].PROPERTIES[0]
           && T100Library::T100String32Tools::compare(info.ITEMS[i].NAME, 200, name)){
            handle.ITEM         = info.ITEMS[i];
            handle.LOCATION     = m_vfs->m_path->m_current;
            handle.OFFSET       = i;
            return T100TRUE;
...
T100BOOL T100VFSItem::first(T100VFS_ITEM_HANDLE& handle)
{
    ...
    for(int i=0;i<16 * 16 - 1;i++){
        if((0 != info.ITEMS[i].PROPERTIES[0])
           && (T100VFS_ITEM_BAD != info.ITEMS[i].PROPERTIES[0])){
            handle.ITEM         = info.ITEMS[i];
            handle.LOCATION     = 0;
            handle.OFFSET       = i;
            return T100TRUE;
