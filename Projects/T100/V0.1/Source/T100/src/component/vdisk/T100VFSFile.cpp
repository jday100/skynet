...
T100VFSFile::T100VFSFile(T100VFS* vfs, T100STRING file)
...
T100BOOL T100VFSFile::create()
{
    ...
    value = T100Library::T100String32Tools::copy(item.ITEM.NAME, fname.to_string32().raw_data(), fname.to_string32().raw_length());
    if(!value){
        return T100FALSE;
    }
...
T100BOOL T100VFSFile::seek(T100DWORD offset)
{
    ...
    for(int i=0;i<count+1;i++){
        if(T100VFS_ITEM_END == location){
            T100VFS_TABLE_HANDLE        item;

            value = m_vfs->m_table->next(item);
            if(!value){
                return T100FALSE;
            }

            if(0 == i){
                m_handle->ITEM.ITEM.LOCATION    = item.CELL;
                m_handle->CURRENT               = item.CELL;

                item.CELL   = T100VFS_ITEM_END;

                value = m_vfs->m_table->save(item);
                if(!value){
                    return T100FALSE;
                }
            ...
            value = m_vfs->m_table->search(location, item);
            if(!value){
                return T100FALSE;
            }
