...
T100BOOL T100VDiskFile::create(T100DWORD length)
{
    ...
    m_info.PHYSICAL_LENGTH          = size + m_info.DATA_LENGTH * 4;
    m_info.FILE_LENGTH              = m_info.PHYSICAL_LENGTH / 4;

    m_stream = T100NEW T100Library::T100FileStream(getName());
    if(!m_stream){
        return T100FALSE;
    }
...
T100BOOL T100VDiskFile::open(T100Library::T100FILE_MODE mode)
{
    T100BOOL        result          = T100TRUE;
    T100BOOL        value;
...
T100DWORD T100VDiskFile::length()
{
    ...
    if(opened()){
        ...
        if(open(T100Library::T100FILE_READ)){
            if(read_head()){
