...
T100VOID T100FontTools::create()
{
    m_opened = T100FALSE;
}

T100VOID T100FontTools::destroy()
{
...
T100BOOL T100FontTools::append(T100WORD begin, T100WORD end)
{
    ...
    T100Component::T100FONTFILE_ROW*   row = T100NEW T100Component::T100FONTFILE_ROW();
...
T100BOOL T100FontTools::remove(T100HWORD index)
{
    if(index < m_rows.size()){
        T100Component::T100FONTFILE_ROW*   row;

        row = m_rows.at(index);
        if(row)T100SAFE_DELETE(row);
...
T100BOOL T100FontTools::createFile(T100STRING file)
{
    T100WORD        amount;
    T100WORD        length;
    ...
    m_file      = T100NEW T100Component::T100FontFile(file);

    if(!m_file->exists()){
        if(m_file->create()){

        }else{
            T100SAFE_DELETE(m_data);
            T100SAFE_DELETE(m_file);
            return T100FALSE;
        }
    }

    m_writer    = m_file->getWriter(&m_font);
    if(!m_writer){
        T100SAFE_DELETE(m_file);
        return T100FALSE;
    }
...
T100BOOL T100FontTools::writeHead()
{
    if(m_convert){
        ...
        name = T100Library::T100Unicode::gbk_to_utf8(m_fontname);
        if(name.empty()){
            return T100FALSE;
        }
        m_file->setFontName(T100Library::T100Unicode::to_wstring(name));
    }else{
        m_file->setFontName(T100Library::T100Unicode::to_wstring(m_fontname));
    }
...
T100BOOL T100FontTools::writeRows()
{
    ...
    for(T100Component::T100FONTFILE_ROW* row : m_rows){
        if(row){
            if(m_writer->write_row(row)){
...
{
    return m_writer->write_item(m_data, m_length);
}
