...
T100WSTRING         T100OSInitTest::m_name                      = L"os.init";

T100OSInitTest::T100OSInitTest(T100Test* parent)
    :T100Test(parent, m_name)
{
    //ctor
}

T100OSInitTest::~T100OSInitTest()
{
    //dtor
}

T100BOOL T100OSInitTest::do_test()
{
...
T100BOOL T100OSInitTest::test_init()
{
    T100BOOL        result          = T100TRUE;
    T100BOOL        value;

    T100Library::T100Log::info(T100TEST_HINT_OS_INIT_TEST_ASSEMBLY_START);
    ...
    show_result(result, T100TEST_HINT_OS_INIT_TEST_ASSEMBLY_STOP);
    return result;
}

T100BOOL T100OSInitTest::test_kernel()
{
    T100BOOL        result          = T100TRUE;
    T100BOOL        value;

    T100Library::T100Log::info(T100TEST_HINT_OS_INIT_TEST_KERNEL_START);
    ...
    source  = T100ThisAppSetup::getTestResources(L"os\\test_kernel.txt");
    target  = T100ThisAppSetup::getTestBuild(L"test_kernel.knl");
    path    = T100ThisAppSetup::getWork(L"BOS");
    ...
    show_result(result, T100TEST_HINT_OS_INIT_TEST_KERNEL_STOP);
    return result;
}

T100BOOL T100OSInitTest::test_vdisk()
{
    T100BOOL        result          = T100TRUE;
    T100BOOL        value;

    T100Library::T100Log::info(T100TEST_HINT_OS_INIT_TEST_VDISK_START);
    ...
    if(result){
        ...
        if(result){
            init    = T100ThisAppSetup::getTestBuild(L"test_kernel.knl");
            cmd     = L"copy " + init + L" first :system:kernel.knl";
            value   = console.exec(cmd.to_wstring());
            if(!value){
                result = T100FALSE;
            }
    ...
    show_result(result, T100TEST_HINT_OS_INIT_TEST_VDISK_STOP);
    return result;
}

T100BOOL T100OSInitTest::test_run()
{
    T100BOOL        result          = T100TRUE;
    T100BOOL        value;

    T100Library::T100Log::info(T100TEST_HINT_OS_INIT_TEST_RUN_START);
    ...
    show_result(result, T100TEST_HINT_OS_INIT_TEST_RUN_STOP);
    return result;
}
