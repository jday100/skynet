...
void T100DXCamera::Init(XMFLOAT3 position)
{
    m_initialPosition = position;
    Reset();
}

void T100DXCamera::SetMoveSpeed(float unitsPerSecond)
{
    m_moveSpeed = unitsPerSecond;
}

void T100DXCamera::SetTurnSpeed(float radiansPerSecond)
{
    m_turnSpeed = radiansPerSecond;
}

void T100DXCamera::Reset()
{
    m_position          = m_initialPosition;
    m_yaw               = XM_PI;
    m_pitch             = 0.0f;
...
void T100DXCamera::Update(float elapsedSeconds)
{
    XMFLOAT3 move(0, 0, 0);
...
XMMATRIX T100DXCamera::GetViewMatrix()
{
    return XMMatrixLookToRH(XMLoadFloat3(&m_position), XMLoadFloat3(&m_lookDirection), XMLoadFloat3(&m_upDirection));
}

XMMATRIX T100DXCamera::GetProjectionMatrix(float fov, float aspectRatio, float nearPlane, float farPlane)
{
    return XMMatrixPerspectiveFovRH(fov, aspectRatio, nearPlane, farPlane);
}

void T100DXCamera::OnKeyDown(UINT8 key)
{
    switch (key)
    {
    case 'W':
...
void T100DXCamera::OnKeyUp(UINT8 key)
{
    switch (key)
    {
    case 'W':
