...
void T100SimpleCamera::Init(XMFLOAT3 position)
{
    m_initialPosition = position;
    Reset();
}

void T100SimpleCamera::SetMoveSpeed(float unitsPerSecond)
{
    m_moveSpeed = unitsPerSecond;
}

void T100SimpleCamera::SetTurnSpeed(float radiansPerSecond)
{
    m_turnSpeed = radiansPerSecond;
}

void T100SimpleCamera::Reset()
{
    m_position = m_initialPosition;
    m_yaw = XM_PI;
    m_pitch = 0.0f;
...
void T100SimpleCamera::Update(float elapsedSeconds)
{
...
XMMATRIX T100SimpleCamera::GetViewMatrix()
{
    return XMMatrixLookToRH(XMLoadFloat3(&m_position), XMLoadFloat3(&m_lookDirection), XMLoadFloat3(&m_upDirection));
}

XMMATRIX T100SimpleCamera::GetProjectionMatrix(float fov, float aspectRatio, float nearPlane, float farPlane)
{
    return XMMatrixPerspectiveFovRH(fov, aspectRatio, nearPlane, farPlane);
}

void T100SimpleCamera::OnKeyDown(WPARAM key)
{
    switch (key)
    {
    case 'W':
...
void T100SimpleCamera::OnKeyUp(WPARAM key)
{
    switch (key)
    {
    case 'W':
