...
const T200AdminDatum = require('../../../models/T200AdminDatum.js');
const T200HomeAdminBiz = require('../../../biz/T200HomeAdminBiz.js');

const T200ListView = require('../../../../library/web/view/T200ListView.js');


async function do_admin_datum_list(request, response, cookie, session, resource) {
    log(__filename, "do_admin_datum_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let datum = new T200AdminDatum();
        ...
        if(T200HttpsForm.verify_id(user_id)){
            datum.flash_admin_paging_fields();
            datum.merge_paging_count = datum.merge_admin_paging_count;
            datum.merge_paging_list = datum.merge_admin_paging_list;
            AdminBiz.paging2(datum).then(function(result){
                let data = {};
                ...
                list_box._list_url = "/admin/datum/list";
                list_box._search_url = "/admin/datum/search";
                list_box._change_status_url = "/admin/datum/list";
...
async function do_admin_datum_search(request, response, cookie, session, resource) {
...
async function do_admin_datum_approve(request, response, cookie, session, resource) {
...
async function do_admin_datum_remove(request, response, cookie, session, resource) {
...
global.action.use_post('/admin/datum/list', do_admin_datum_list);
global.action.use_post('/admin/datum/search', do_admin_datum_search);
global.action.use_post('/admin/datum/approve', do_admin_datum_approve);
global.action.use_post('/admin/datum/remove', do_admin_datum_remove);