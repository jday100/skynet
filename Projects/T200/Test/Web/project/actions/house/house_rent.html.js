...
async function do_house_rent_reply(request, response, cookie, session, resource) {
    log(__filename, "do_house_rent_reply");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let house = new T200UserHouseRent();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
        ...
        if(T200HttpsForm.verify_id(house.user_id)
            ...
            UserBiz.append(house.merge_user_insert()).then(function(){
                do_house_rent_board_panel(house, UserBiz, cookie, resource).then(function(result){
                    response.type("json");
                    resolve(result);
                }, function(){
                    response.type("json");
...
function do_house_rent_board_panel(house, UserBiz, cookie, resource) {
    log(__filename, "do_house_rent_board_panel");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let item = cookie.get("id");
...
global.action.use_post('/house/rent/reply', do_house_rent_reply);