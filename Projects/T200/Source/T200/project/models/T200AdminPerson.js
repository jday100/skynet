...
class T200AdminPerson extends T200HomeAdminModel {
    ...
    set_item_left() {
        return [
            ['Username', 'username', true, '/admin/person/profile.html', 'list_title']
        ];
    }
    ...
    list_status_fields() {
        return "user_id as id, status, username, create_time";
    }
    ...
    merge_status_update() {
        return `update ${this._table} set status = ${this.status} where user_id in (${this.ids})`;
    }
    ...
    list_fields() {
        return "user_id, status, title, create_time";
    }

    fulltext_result_fields() {
        return "user_id, title, t1.create_time";
    }
