...
async function do_trading_buy_region_list(request, response, cookie, session, resource) {
    log(__filename, "do_trading_buy_region_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(T200HttpsForm.verify_id(region_id)){
            ...
            UserBiz.paging2(trading).then(function(result){
                ...
                return view.render_file("trading/buy_region.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
async function do_trading_buy_region_search(request, response, cookie, session, resource) {
    log(__filename, "do_trading_buy_region_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let trading = new T200UserTradingBuy();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/trading/buy/region/list', do_trading_buy_region_list);
global.action.use_post('/trading/buy/region/search', do_trading_buy_region_search);