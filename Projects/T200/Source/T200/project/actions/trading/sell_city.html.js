...
async function do_trading_sell_city_list(request, response, cookie, session, resource) {
    log(__filename, "do_trading_sell_city_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(T200HttpsForm.verify_id(city_id)){
            ...
            UserBiz.paging(trading).then(function(result){
                ...
                return view.render_file("trading/sell_city.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
global.action.use_post('/trading/sell/city/list', do_trading_sell_city_list);