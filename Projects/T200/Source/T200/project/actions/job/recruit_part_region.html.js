...
async function do_job_recruit_part_region_search(request, response, cookie, session, resource) {
    log(__filename, "do_job_recruit_part_region_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let job = new T200UserJobRecruit();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/job/recruit/part/region/search', do_job_recruit_part_region_search);