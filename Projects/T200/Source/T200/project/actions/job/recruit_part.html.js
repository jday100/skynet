...
async function do_job_recruit_part_list(request, response, cookie, session, resource) {
    log(__filename, "do_job_recruit_part_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(true){
            ...
            UserBiz.paging2(job).then(function(result){
                ...
                return view.render_file("job/recruit_part.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
async function do_job_recruit_part_search(request, response, cookie, session, resource) {
    log(__filename, "do_job_recruit_part_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let job = new T200UserJobRecruit();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/job/recruit/part/list', do_job_recruit_part_list);
global.action.use_post('/job/recruit/part/search', do_job_recruit_part_search);