...
async function do_job_wanted_full_city_list(request, response, cookie, session, resource) {
    log(__filename, "do_job_wanted_full_city_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(T200HttpsForm.verify_id(city_id)){
            ...
            UserBiz.paging2(job).then(function(result){
                ...
                return view.render_file("job/wanted_full_city.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
global.action.use_post('/job/wanted/full/city/list', do_job_wanted_full_city_list);