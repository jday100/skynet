...
async function do_job_wanted_full_region_list(request, response, cookie, session, resource) {
    log(__filename, "do_job_wanted_full_region_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(T200HttpsForm.verify_id(region_id)){
            ...
            UserBiz.paging2(job).then(function(result){
                ...
                return view.render_file("job/wanted_full_region.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
async function do_job_wanted_full_region_search(request, response, cookie, session, resource) {
    log(__filename, "do_job_wanted_full_region_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let job = new T200UserJobWanted();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/job/wanted/full/region/list', do_job_wanted_full_region_list);
global.action.use_post('/job/wanted/full/region/search', do_job_wanted_full_region_search);