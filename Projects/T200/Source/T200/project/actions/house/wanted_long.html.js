...
async function do_house_wanted_long_search(request, response, cookie, session, resource) {
    log(__filename, "do_house_wanted_long_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        if(global.setup.server){
        ...
        if(T200HttpsForm.verify_text(search)){
            ...
            house.merge_fulltext_count = house.merge_user_fulltext_long_count;
            house.merge_fulltext_list = house.merge_user_fulltext_long_list;
...
global.action.use_post('/house/wanted/long/search', do_house_wanted_long_search);