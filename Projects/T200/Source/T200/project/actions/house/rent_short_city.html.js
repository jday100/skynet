...
async function do_house_rent_short_city_search(request, response, cookie, session, resource) {
    log(__filename, "do_house_rent_short_city_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let house = new T200UserHouseRent();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/house/rent/short/city/search', do_house_rent_short_city_search);