...
async function do_house_wanted_short_search(request, response, cookie, session, resource) {
    log(__filename, "do_house_wanted_short_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        if(global.setup.server){
...
global.action.use_post('/house/wanted/short/list', do_house_wanted_short_list);
global.action.use_post('/house/wanted/short/search', do_house_wanted_short_search);