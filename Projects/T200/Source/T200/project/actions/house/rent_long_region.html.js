...
async function do_house_rent_long_region_list(request, response, cookie, session, resource) {
    log(__filename, "do_house_rent_long_region_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(T200HttpsForm.verify_id(region_id)){
            ...
            UserBiz.paging2(house).then(function(result){
                ...
                return view.render_file("house/rent_long_region.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
async function do_house_rent_long_region_search(request, response, cookie, session, resource) {
    log(__filename, "do_house_rent_long_region_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let house = new T200UserHouseRent();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/house/rent/long/region/search', do_house_rent_long_region_search);