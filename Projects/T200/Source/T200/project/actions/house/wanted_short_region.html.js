...
async function do_house_wanted_short_region_list(request, response, cookie, session, resource) {
    log(__filename, "do_house_wanted_short_region_list");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        ...
        if(T200HttpsForm.verify_id(region_id)){
            ...
            UserBiz.paging2(house).then(function(result){
                ...
                return view.render_file("house/wanted_short_region.ejs", data).then(function (value) {
                    response.type("json");
                    resolve(value);
                }, function (err) {
                    response.type("json");
...
async function do_house_wanted_short_region_search(request, response, cookie, session, resource) {
    log(__filename, "do_house_wanted_short_region_search");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let house = new T200UserHouseWanted();
        let UserBiz = new T200HomeUserBiz(request, cookie, session);
...
global.action.use_post('/house/wanted/short/region/list', do_house_wanted_short_region_list);
global.action.use_post('/house/wanted/short/region/search', do_house_wanted_short_region_search);